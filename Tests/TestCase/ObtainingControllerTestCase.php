...
/**
 * Classe de base pour les tests unitaires du contrôleur ObtainingController.
 *
 * Cette classe :
 ...
 */
abstract class ObtainingControllerTestCase extends TestCase
{
     ...
     * - La mise en place d'une base SQLite en mémoire avec la table zell_obtaining
    ...
    protected function setUp(): void
    {
        ...
        @mkdir("{$this->viewPath}/obtaining", 0777, true);
        @mkdir("{$this->viewPath}/templates", 0777, true);
        @mkdir("{$this->viewPath}/partials", 0777, true);

        file_put_contents("{$this->viewPath}/obtaining/add-obtaining.php", '<form>add</form>');
        file_put_contents("{$this->viewPath}/obtaining/edit-obtaining.php", '<form>edit</form>');
        file_put_contents("{$this->viewPath}/templates/default.php", '<html><?= $title ?><?= $content ?></html>');
        file_put_contents("{$this->viewPath}/obtaining/delete-obtaining-confirm.php", '<form>confirm delete</form>');
        file_put_contents("{$this->viewPath}/obtaining/obtaining-list.php", '<ul><li>Obtaining</li></ul>');
        ...
        $this->pdo->exec('
            CREATE TABLE zell_obtaining (
                id_obtaining INTEGER PRIMARY KEY AUTOINCREMENT,
                obtaining TEXT NOT NULL UNIQUE
            )
        ');
